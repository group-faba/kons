...
REDIRECT_URI = os.getenv('REDIRECT_URI')  # https://<домен>/oauth2callback
APP_URL      = os.getenv('APP_URL')       # https://<домен>
...
# Сразу регистрируем webhook (под Gunicorn этот код выполнится при импорте)
...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    if not get_creds(update.effective_user.id):
        return await cmd_link(update, ctx)
