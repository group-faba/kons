...
# Записываем client_secrets.json из ENV
...
async def cb_industry(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    context.user_data['industry'] = ind
    flt = [
        s for s in SPECIALISTS
        if s['region']==context.user_data['region'] and s['industry']==ind
    ]
...
async def cb_date(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    for h in range(9, 18):
        ...
        if any(
            datetime.fromisoformat(b['start'].rstrip('Z')) <= t0 < datetime.fromisoformat(b['end'].rstrip('Z'))
            for b in busy
        ):
            continue
...
async def cb_time(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.callback_query.edit_message_text(
        f"Запись подтверждена: {spec['name']}, {d} в {t0}"
    )
...
def main():
    ...
    while True:
        try:
            ...
            logging.warning("Polling timed out — перезапускаем через 1 сек.")
            time.sleep(1)
